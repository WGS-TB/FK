...
%&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
%
% This function computes dual of a given dnf using hash table and finding mutiple conflict assignments.
...
% n_clause = 1;
% thresh = 3;
% [cnf, cput, cnf_len] = FK_Dualization_MH( dnf, n_clause, thresh ) == > cnf = [1, 1, 0, 0, 0; 1, 0, 1, 0, 0]
%
% Just to see if the returned cnf is correct: 
% FK(cnf, dnf, 'mostFreq') % it returns [] that means the returned cnf and dnf are dual.

function [cnf, cput, cnf_len]= FK_Dualization_MH( dnf, n_clause, thresh )
...
function CA = FK_MH(cnf, dnf, split_method)

global cellHash cellSize Hash_thresh

% cellHash is the hash table
% cellSize indicates to the first blank record in cellHash
...
if min(nD, nC) <= 2
    ...
    if mu_frequent_in_A(split_var, dnf, cnf)% if split variable is at most mu-frequent in DNF (line 7)
        ...
        if (~cond || strcmp(CA, 'None'))
            CA = FK_MH(tempCNF, D_x_1, split_method ); % line 8
            if(~isempty(key1))
        ...
        for i=1: size(C_x_0,1) %line 10
            ...
            if(isempty(D_cx_0) && isempty(C_cx_1))
                ...
                if (~cond || strcmp(CA, 'None'))
                    CA = FK_MH(C_cx_1, D_cx_0, split_method); %line 11
            ...
            if (~cond || strcmp(CA, 'None'))
                CA = FK_MH(C_x_1, tempDNF, split_method); %line 14
            ...
            for i=1: size(D_x_0,1) %line 16
                ...
                if(isempty(D_mx_1) && isempty(C_mx_0))
                    ...
                    if (~cond || strcmp(CA, 'None'))
                        CA = FK_MH(C_mx_0, D_mx_1, split_method); %line 17
            ...
            if (~cond || strcmp(CA, 'None'))
                CA = FK_MH(tempCNF, D_x_1, split_method);  %line 20
            ...
            if(isempty(CA)) % line 21
                ...
                if (~cond || strcmp(CA, 'None'))
                    CA = FK_MH(C_x_1, tempDNF, split_method); % line 22
