...
%&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
%
% This script computes cputimes for computing dual of each problem using timeit() function for different methods.
% Running this script takes time because of timeit() function that automatically runs every function several times.
%
% Please note that when timeit() is used, 'tic() toc()' functions in dualization algorithms must be commented.
clear all
...
for nn=1:numel(models)
    ...
    %%%%%%%%%%%%%%%%%%%%%%
    rng(6)
    f = @() FK_Dualization( dnf, 1 );
    FK_cpu = timeit(f);
    
    %%%%%%%%%%%%%%%%%%%%%%
    
    rng(6)
    f = @() FK_Dualization_M( dnf, 1 );
    FKM_cpu = timeit(f);
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%
    rng(6)
    f = @() FK_Dualization_R( dnf, 1 );
    R_cpu = timeit(f);
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%
    rng(6)
    f = @() FK_Dualization_MHR( dnf, 1, 0);
    HR0_cpu = timeit(f);
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
    rng(6)
    f = @() FK_Dualization_MHR( dnf, 1, 3);
    HR3_cpu = timeit(f);
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%

    rng(6)
    f = @() FK_Dualization_MHCR(dnf, 1, 0);
    HCR0_cpu = timeit(f);
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%

    rng(6)
    f = @() FK_Dualization_MHCR(dnf, 1, 3);
    HCR3_cpu = timeit(f);
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%% 
    
    cputimes(nn,:) = [FK_cpu, FKM_cpu, R_cpu, ...
        HR0_cpu, HR3_cpu, HCR0_cpu, HCR3_cpu];
    
end
methods = {'FK', 'FKM', 'FKR', 'FKMHR0', 'FKMHR3', 'FKMHCR0', 'FKMHCR3'};
save('CPUtimes_test.mat', 'models', 'methods', 'cputimes')
