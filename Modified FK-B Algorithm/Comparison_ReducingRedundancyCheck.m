...
%&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
%
% This script compares FK_dualization and FK_dualization_R in terms of the
% number of redundancy checks

clear all

global redundancy_counter
...
for nn=1:numel(models)
    ...
    rng(6)
    [cnf, cput, cnf_len]= FK_Dualization( dnf, 1);
    ...
    call_counter = 0;
    rng(6)
    [FKR_cnf, FKR_cpu, FKR_len]= FK_Dualization_R( dnf, 1 );
    redundancy(nn,2) = redundancy_counter;
    
end
save('Check_FKandFKR.mat', 'redundancy')

figure
b=bar(redundancy(:,[1, 2]));
b(1,1).FaceColor = 'b';
...
xticklabels(models)
xtickangle(55)
ylabel('Num of redundancy check')

saveas(gcf, 'RedundancyCheck.pdf')
